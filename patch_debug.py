...
new_logic = """  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!activeGroup || !user || !profile) {
        toast.error("Missing context (group/user/profile)");
        return;
    }
    ...
    reader.onload = (event) => {
        ...
        img.onerror = () => toast.error("Error loading image for compression", { id: 'image-upload' });
...
content = re.sub(
    r'  const handleFileChange = async \(e: React\.ChangeEvent<HTMLInputElement>\) => \{.*?(?=  const fetchGroups = async \(\) => \{)',
    new_logic + '\n\n',
    content,
    flags=re.DOTALL
)
